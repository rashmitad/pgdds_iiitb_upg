...
# Store the names of those movies in last_10 vector (in the same order)
	...
	last_10 <-bollywood[(nrow(bollywood)-10):nrow(bollywood),]$Movie 
...
# Store the result in na_bollywood vector
	...
	na_bollywood <- colSums(is.na(bollywood))
...
# Store the names of 4 movies in same sequence in movie_result vector
	...
	matchMaxColl<-function(ITER)
	{
	  return(bollywood[which.max(bollywood[,ITER]),1])
	}
	
	movie_result<-sapply(which(sapply(bollywood,is.numeric)==T),matchMaxColl)
